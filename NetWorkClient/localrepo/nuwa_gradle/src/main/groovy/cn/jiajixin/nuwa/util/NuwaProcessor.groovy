            ...
            if (inputFile.isDirectory()) {
                ...
                if (NuwaSetUtils.isIncluded(path, includePackage)) {
                    if (!NuwaSetUtils.isExcluded(path, excludePackage, excludeClass)) {
                        ...
                        def hash = DigestUtils.shaHex(bytes)
                        // 由于realm的影响,这里要对对原path做个转换,使得记录下来的路径保持正常类的路径
                        def _path = path.substring(path.indexOf(includePackage.iterator().next()))
                        hashFile.append(NuwaMapUtils.format(_path, hash))
                        if (NuwaMapUtils.notSame(map, _path, hash)) {
                            println("patch class:" + _path)
                            NuwaFileUtils.copyBytesToFile(inputFile.bytes, NuwaFileUtils.touchFile(patchDir, _path))
                        }
                    }
                }
            }
